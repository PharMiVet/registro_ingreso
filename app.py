...
sedes = {'Chacarilla':{'lat':-12.1126478,'long':-76.9914866,'ingreso':'09:05','salida':'19:00'},
            'Limatambo':{'lat':-12.1116881,'long':-77.0037359,'ingreso':'09:05','salida':'19:00'},
            'Bellavista':{'lat':-12.0621377,'long':-77.0959635,'ingreso':'09:05','salida':'19:00'},#
            'Almacen':{'lat':-12.0928895,'long':-77.0328699,'ingreso':'09:05','salida':'19:00'},#//
              'Cayetano':{'lat':-12.0239117,'long':-77.05775,'ingreso':'08:05','salida':'19:00'},#//ajustado
              'Izaguirre':{'lat':-11.9901055,'long':-77.0661156,'ingreso':'09:05','salida':'19:00'},#//
              'Belaunde':{'lat':-11.935437,'long':-77.0554565,'ingreso':'09:05','salida':'19:00'},#//,
              'Honorio Delgado':{'lat':-12.0254019,'long':-77.0568629,'ingreso':'08:05','salida':'19:00'},#//,
              'Estación Los Jardines':{'lat':-12.0063581,'long':-77.0071405,'ingreso':'09:05','salida':'19:00'},#//
              'Bolívar':{'lat':-12.0723052,'long':-77.0644842,'ingreso':'09:05','salida':'19:00'},#//
              'Huaylas':{'lat':-12.1738181,'long':-77.0197543,'ingreso':'09:05','salida':'19:00'},#//
              'San Juan de Miraflores':{'lat':-12.1579779,'long':-76.9732791,'ingreso':'09:05','salida':'19:00'},#//
              'San Juan de Miraflores 2':{'lat':-12.0782848,'long':-77.037568,'ingreso':'09:05','salida':'19:00'},#//	
              'San Germán':{'lat':-12.0198906,'long':-77.0761357,'ingreso':'09:05','salida':'19:00'},#//
              'Sagitario':{'lat':-12.1625098,'long':-76.9956862,'ingreso':'09:05','salida':'19:00'},#//
              'Villa Sol':{'lat':-11.9586625,'long':-77.0760639,'ingreso':'09:05','salida':'19:00'},#//
...
sede_encontrada = 'Ninguna'
for sede in sedes:
    lat_sede = sedes[sede]['lat'];
    lon_sede = sedes[sede]['long'];
    distancia = getDistanceBetweenPoints(lat, lon, lat_sede, lon_sede);
    if distancia < 120:
...
st.sidebar.image('https://www.pharmivet.pe/web/image/website/1/logo/PharMiVet%20-%20Farmacias%20Veterinarias?unique=7fdc782',
                 width=200)
st.sidebar.markdown("""
                    > (https://pharmivet.pe)
                    """)
...
user_color      = '#65B32E'
title_webapp    = "PharMiVet Registro Colaboradores"
...
html_temp = f"""
            ...
            <h3 style="color:white;text-align:center;">{title_webapp} </h3>
            </div>
            """
st.markdown(html_temp, unsafe_allow_html=True)

###################### Defining Static Paths ###################4
if st.sidebar.button('Borrar todos los registros'):
...
def main():
    ###################################################
    st.sidebar.header("PharMiColaborador")
    st.sidebar.info("Este app sirve para los colaboradores registre su ingreso y salida ")
    ###################################################

    selected_menu = option_menu(None,
        ['Registro ingreso/salida', 'Matricular colaborador' ,'Ver registros historicos'],
        icons=['camera', "clock-history", 'person-plus'],
        ## icons from website: https://icons.getbootstrap.com/
        menu_icon="cast", default_index=0, orientation="horizontal")

    if selected_menu == 'Registro ingreso/salida':
        ...
        if img_file_buffer is not None:
            ...
            with open(os.path.join(VISITOR_HISTORY,
                ...
                if max_faces > 0:
                    ...
                    similarity_threshold = col2.slider('Selectcionar umbra para similaridad',
                    ...
                    if ((col1.checkbox('Click para registrar!')) & (len(face_idxs)>0)):
                    ...
                    st.error('No se ha detecto el rostro de una persona.')

    if selected_menu == 'Ver registros historicos':
        view_attendace()

    if selected_menu == 'Matricular colaborador':
        col1, col2, col3 = st.columns(3)

        face_name  = col1.text_input('Nombre:', '')
        pic_option = col2.radio('Registrar por primera vez',
                                options=[
                                         #"Upload a Picture",
                                         "Tomar una foto"])
        ...
        if pic_option == 'Tomar una foto':
            img_file_buffer = col3.camera_input("Tomar una foto")
            if img_file_buffer is not None:
                # To read image file buffer with OpenCV:
